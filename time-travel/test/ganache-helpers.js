...
const LogNetwork = Debug("network");
...
const makeNetworkSnapshot = () => 
  rpcPromise({ method: 'evm_snapshot' });
...
const getLatestBlockTimestamp = async () => {
  ...
  LogNetwork("getLatestBlockTimestamp: %s", new Date(timestamp*1000));
  return timestamp;
}
